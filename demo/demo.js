(function() {
  var click_handler, initialize, inject_box, is_box_visible, place_guider_note, place_intro_guider_note, root;
  ...
  initialize = function() {
    ...
    async_if(is_box_visible, 'red').then(function(status) {}, function(status) {
      return alert("" + status + " with failure ....");
    });
    return $.when(async_if(is_box_visible, 'green'), async_if(is_box_visible, 'red')).then(function() {
      inject_box('blue');
